...
import { fetchChatUsers, fetchChats, sendMessage } from '../../redux/slices/studentSlice';
import { initializeSocket } from '../../apis/endpoints/studentEndpoint';
...
const ChatInterface = () => {
  ...
  const { chatUsers = [], chats = [], loading, error } = useSelector((state) => state.student);
  ...
  const filteredChats = useMemo(() => {
    ...
    console.log('=== STUDENT CHAT FILTERING ===');
    ...
    <div className="container mx-auto p-4 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
      <div className="bg-white shadow-2xl rounded-2xl overflow-hidden flex flex-col md:flex-row h-[85vh] border border-gray-200">
        ...
        <div className="w-full md:w-1/3 border-b md:border-r border-gray-200 flex flex-col">
          <div className="flex justify-between items-center p-5 bg-gradient-to-r from-green-600 to-emerald-700 text-white">
            ...
            <button
              ...
              disabled={refreshing}
              className="p-2 rounded-full hover:bg-green-800 transition-colors duration-200"
              title="Refresh chats"
            >
          ...
          <div className="text-xs p-2 bg-yellow-50 text-yellow-800 border-b border-yellow-200">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-xs px-2 py-1 rounded-full bg-emerald-100 text-emerald-600 capitalize mt-1 inline-block">
        ...
        <div className="w-full md:w-2/3 flex flex-col">
          {selectedUser ? (
            <>
              {/* Chat Header */}
              <div className="flex justify-between items-center p-4 bg-gradient-to-r from-green-50 to-emerald-100 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <FaSync className={`text-green-600 ${refreshing ? 'animate-spin' : ''}`} />
                  </button>
                </div>
              </div>

              {/* Debug info for current chat */}
              {filteredChats.length > 0 && (
                <div className="text-xs p-2 bg-green-50 text-green-800 border-b border-green-200">
                  Showing {filteredChats.length} messages with {selectedUser.firstName}
                </div>
              )}
              ...
              <div
                ref={chatRef}
                className="flex-grow p-4 overflow-y-auto bg-gradient-to-b from-white to-green-50 space-y-3"
              >
                {sortedChats.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-gray-500">
                    <div className="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center mb-4">
                      <FaExclamationCircle className="text-3xl text-green-400" />
                    </div>
                    <p className="text-lg font-semibold">No messages yet</p>
                    <p className="text-sm mt-1">Start the conversation!</p>
                    <div className="mt-4 text-xs text-gray-400">
                      <p>Your ID: {user?.id}</p>
                      <p>Their ID: {selectedUser.id}</p>
                    </div>
                  </div>
                ) : (
              ...
              >
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-grow border border-gray-300 p-4 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-200"
                    disabled={selectedUser.id === user?.id}
                  />
                  <button
                    type="submit"
                    disabled={!message.trim() || selectedUser.id === user?.id || loading}
                    className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4 rounded-xl hover:from-green-700 hover:to-emerald-700 transition duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <FaPaperPlane /> 
            ...
            <div className="flex items-center justify-center h-full text-gray-500 bg-gradient-to-br from-green-50 to-emerald-50">
              <div className="text-center p-8">
                <div className="w-24 h-24 mx-auto bg-gradient-to-br from-green-100 to-emerald-200 rounded-full flex items-center justify-center mb-6">
                  <FaExclamationCircle className="text-4xl text-green-400" />
                </div>
                <h3 className="text-2xl font-semibold text-gray-700 mb-2">Student Chat</h3>
                <p className="text-gray-600 mb-4">Select a user to start chatting</p>
                <div className="text-sm text-gray-500">
                  <p className="mb-1">Your Student ID: {user?.id || 'Loading...'}</p>
                  <p className="mb-1">• Chat with teachers and peers</p>
                  <p className="mb-1">• Discuss academic matters</p>
