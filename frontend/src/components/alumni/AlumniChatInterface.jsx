    ...
    <div className="container mx-auto p-4 bg-gray-100 min-h-screen">
      <div className="bg-white shadow-xl rounded-xl overflow-hidden flex flex-col md:flex-row h-[80vh]">
        <div className="w-full md:w-1/3 border-b md:border-r md:border-b-0 border-gray-200 overflow-y-auto">
          <div className="flex justify-between items-center p-4 bg-gradient-to-r from-navy-800 to-navy-600 text-white">
          ...
          {loading && (
            <div className="flex justify-center py-4">
              <FaSpinner className="animate-spin text-3xl text-navy-600" />
            </div>
          )}
          {error && (
          ...
          <ul className="divide-y divide-gray-200">
            {allChatUsers.map((u) => (
              <li
                key={u.id}
                className={`p-4 cursor-pointer transition-all duration-200 hover:bg-navy-50 ${
                  selectedUser?.id === u.id ? 'bg-navy-100 border-l-4 border-navy-600' : ''
                }`}
                onClick={() => setSelectedUser(u)}
              >
                <div className="flex items-center">
                  <div className="w-10 h-10 rounded-full bg-navy-200 flex items-center justify-center text-navy-600 font-bold">
                    {u.firstName[0]}
                  </div>
                  <div className="ml-3">
        ...
        <div className="w-full md:w-2/3 flex flex-col">
          {selectedUser ? (
            <>
              <div className="flex justify-between items-center p-4 bg-gradient-to-r from-navy-600 to-navy-800 text-white">
              ...
              >
                <input
                  ...
                  className="flex-grow border border-gray-300 p-3 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-navy-600 transition duration-200"
                />
                <button
                  type="submit"
                  className="bg-navy-600 text-black p-3 rounded-r-lg hover:bg-navy-700 transition duration-200 flex items-center bg-blue-500"
                >
            ...
            <div className="flex items-center justify-center h-full text-gray-500 bg-gray-50">
              <div className="text-center">
                <FaExclamationCircle className="text-5xl mb-4 text-navy-300" />
